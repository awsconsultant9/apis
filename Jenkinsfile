pipeline {
    ...
    stages {
        ...
        stage('Checkout Code') {
            steps {
                git 'https://github.com/awsconsultant9/apis.git'
            }
        }
        ...
        stage('Deploy to EC2-B') {
            steps {
                ...
                ssh -i $SSH_KEY -o StrictHostKeyChecking=no $TARGET_HOST "mkdir -p $APP_DIR"

                echo "[2] Copy project files from EC2-A to EC2-B..."
                rsync -avz -e "ssh -i $SSH_KEY -o StrictHostKeyChecking=no" --exclude='.venv' ./ $TARGET_HOST:$APP_DIR
                '''
            }
        }
        ...
        stage('Install Dependencies & Start FastAPI Server') {
            steps {
                sh '''
                ssh -i $SSH_KEY -o StrictHostKeyChecking=no $TARGET_HOST << EOF
                    cd $APP_DIR
